...
class Sampler:
    ...
    def dtype(self) -> jnp.dtype:
        return jax.tree_util.tree_leaves(nnx.to_flat_state(self._transformer_state))[
            0
        ].dtype
    ...
    def mask_tokens_after_eos_ids(self, token_buffer):
        ...
        mask = jnp.less_equal(jnp.arange(token_buffer.shape[-1]), eos_indices[:, None])
    ...
    def _sample_fn(
        ...
        return jax.lax.while_loop(cond_fn, sample_with_params, initial_sampling_state)
